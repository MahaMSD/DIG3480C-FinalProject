...
Kill.prototype.type = "kill";

function Bomb(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1, 1);
   if (ch == "b") {
    // Drop bomb. Repeat back to this pos.
    this.speed = new Vector(0, 7);
    this.repeatPos = pos; 
  }
}
...
function Fire(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1, 1);
   if (ch == "n") {
    // Drop fire. Repeat back to this pos.
...
Kill.prototype.act = function(step, level) {
...
Rocket.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Fire.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "kill" && this.status == null){
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "bomb" && this.status == null){
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "ghost" && this.status == null){
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "rocket" && this.status == null){
    this.status = "lost";
    this.finishDelay = 1;
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost") {
        ...
        alert("Good Job! Next level will start now");
        startLevel(n + 1); }
      else
        alert("You have completed all the levels! congratulations!");
    });
  }
  startLevel(0);
}
